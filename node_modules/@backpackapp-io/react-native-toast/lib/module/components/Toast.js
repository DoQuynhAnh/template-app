...
export const Toast = ({
  ...
  const [toastHeight, setToastHeight] = useState(toast?.height ? toast.height : DEFAULT_TOAST_HEIGHT);
  const [toastWidth, setToastWidth] = useState(toast?.width ? toast.width : width - 32 > 360 ? 360 : width - 32);
  const startingY = useMemo(() => toast.position === ToastPosition.TOP ? -(toast.height || DEFAULT_TOAST_HEIGHT) - insets.top - 50 : height - insets.bottom - Platform.select({
    ios: 0,
    default: 32
