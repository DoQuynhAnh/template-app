...
export const TabCancelled = () => {
  const { isFetching, data, refetch } = getOrders(OrderStatusEnum.CANCEL);

  // func
  const renderItemOrder = ({ index, item }: ListRenderItemInfo<IOrder>) => {
    return (
      <ShopGroupOrder type={TABS.TAB_CANCELLED} orderItems={item} key={index} />
    );
  };
