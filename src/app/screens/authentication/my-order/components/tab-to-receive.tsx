...
export const TabToReceive = () => {
  const { isFetching, data, refetch } = getOrders(OrderStatusEnum.PREPARE);

  // func
  const renderItemOrder = ({ index, item }: ListRenderItemInfo<IOrder>) => {
    return (
      <ShopGroupOrder type={TABS.TAB_RECEIVE} orderItems={item} key={index} />
    );
  };
